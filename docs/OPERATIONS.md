...
- **Mode (Linux)** — **Bar panel** (default on Omarchy): the `fernando.ai-pulse` bar widget opens the leaderboard as a drop-down panel on click (`j`/`k`, Enter opens the model page, `R` refresh, `D` dashboard, `S` settings, Esc closes); no window is created and nothing on screen moves. **Floating window**: the Electron widget, floated and pinned by the Hyprland rule shipped in `packages/widget/linux/hypr/ai-pulse.lua` (installed by `npm run linux:install`) and docked to the right edge below the bar.
- **Monitor** (Linux, floating window only) — pin the leaderboard to a named output (`DP-2`) instead of letting it follow the focused one. It becomes an `o.window(..., { monitor = … })` rule in `~/.config/hypr/ai-pulse-dock.lua`; the window is reopened so the map-time rule applies.
- **Always-on-top** — keep it above other windows. **Windows only**: on Linux/Hyprland the compositor owns stacking, so the toggle is disabled.
- **Keep tiled windows beside the widget** (Linux, floating window only, off by default) — the app writes `~/.config/hypr/ai-pulse-dock.lua` (required by `ai-pulse.lua`) with an extra `gaps_out` on the dock side for the widget's monitor (`workspace = "m[<monitor>]"`) and runs `hyprctl reload config-only`, so windows tile beside the widget instead of underneath; the bar is untouched. Removed when the widget is hidden, on quit, and on the next start. Both app windows carry the window class `ai-pulse`.
